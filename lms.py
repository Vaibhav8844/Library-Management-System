...
def clear():
     for _ in range(7):    #for spacing between menu
        print()

def add_book():
  conn = pymysql.connect(host='localhost', database='library', user='root', password='root')
  cursor = conn.cursor()
...
def modify_member():
    ...
    cursor = conn.cursor()
    clear()
    print('\n\n\t\t\t\t\tModify Memeber Information Screen \n\n')
    print('-'*120)
    print('\n1. Name')
    print('\n2. Class')
    print('\n3. address')
...
def report_book_list():
    ...
    for record in records:
       print(record)
       conn.commit()
    sql1='select count(title) from book group by author;'
    cursor.execute(sql1)
    y=cursor.fetchall()
    v=[]
    for i in y:
      v.append(i[0])
    conn.commit()
    sql2='select distinct(author) from book;'
    cursor.execute(sql2)
    x=cursor.fetchall()
    u=[]
    for j in x:
      u.append(j[0])
    conn.commit()
    plt.bar(u,v,width=0.5,color='b')
    plt.xlabel("Authors")
    plt.ylabel("Number of books")
    plt.title("Authors and Their Number of books")
    plt.show()
    conn.commit()
    conn.close()
